package com.cjy.retrofitlibrary.download;//package com.mvpframe.capabilities.http.download;
//
...
//        RetrofitDownload.get().removeDownload(bean, true);//移除下载
//        RetrofitDownload.get().startDownload(bean);//开始下载
//    }
//
//    @Override
...
//            //设置回调(需要在 RetrofitDownload.get() 的所有操作之前，才能正确回调)
//            item.setCallback(new DownloadCallback<DownloadBean>() {
//                @Override
//                public void onProgress(DownloadModel.State state, long currentSize, long totalSize, float progress) {
...
//            if (item.getState() == DownloadModel.State.LOADING) {//正在下载中，恢复下载进度
//                RetrofitDownload.get().startDownload(item);
//            }
//
//            //进度
...
//                        case LOADING:
//                        case ERROR:
//                            RetrofitDownload.get().stopDownload(item);
//                            break;
//                        case FINISH:
...
//        List<DownloadBean> localList = RetrofitDownload.get().getDownloadList(DownloadBean.class);
